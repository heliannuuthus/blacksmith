...
# 加载 .env 文件（支持 GitHub、OpenRouter 和百炼凭证）
load_dotenv()
...
PROVIDER_DEFAULTS = {
    "openrouter": ("moonshotai/kimi-k2.6", "moonshotai/kimi-k3"),
    "bailian": ("qwen3.6-plus", "qwen3.7-max"),
    "custom": ("qwen3.7-max", "qwen3.7-max"),
}
LLM_CONCURRENCY = int(os.environ.get("LLM_CONCURRENCY", "5"))  # LLM 调用并发度
...
TAG_TAXONOMY_PROMPT = (
    "可选标签（只能从下列词表选择，可多选）：\n"
    "- cuisines: 家常菜、川菜、湘菜、粤菜、鲁菜、苏菜、浙菜、闽菜、徽菜、东北菜、"
    "西北菜、云贵菜、港澳台、日韩料理、东南亚菜、南亚菜、西餐、其他\n"
    "- flavors: 清淡、咸鲜、香辣、麻辣、酸辣、酸甜、甜味、鲜香、蒜香、葱香、酱香、"
    "五香、奶香、烟熏、酒香\n"
    "- scenes: 早餐、午餐、晚餐、夜宵、快手菜、下饭菜、宴客、聚餐、一人食、减脂、"
    "儿童、节日、甜点、饮品、调味料"
)
...
def get_model(stage: str) -> str:
    """Resolve the model for the process or refine stage."""
    provider = get_provider()
    process_default, refine_default = PROVIDER_DEFAULTS[provider]
    default = process_default if stage == "process" else refine_default
    return os.environ.get(f"{provider.upper()}_{stage.upper()}_MODEL", default)
...
def get_llm(model: str, temperature: float = 0.2) -> ChatOpenAI:
    ...
    if provider == "bailian":
        ...
        base_url = os.environ.get("BAILIAN_BASE_URL", BAILIAN_BASE_URL)
        ...
        default_headers = {
            "HTTP-Referer": os.environ.get(
                "OPENROUTER_HTTP_REFERER", "https://github.com/heliannuuthus/blacksmith"
            ),
            "X-OpenRouter-Title": os.environ.get("OPENROUTER_APP_TITLE", "Blacksmith HowToCook"),
        }
        extra_body = None
    else:
        api_key = os.environ.get("CUSTOM_API_KEY")
        if not api_key:
            raise ValueError("CUSTOM_API_KEY required")
...
def invoke_llm(llm: ChatOpenAI, messages: list, context: str = "") -> str:
    ...
    for attempt in range(1, LLM_MAX_RETRIES + 1):
        try:
            ...
            # DEBUG 级别：详细信息
            logger.debug(
                "[LLM]   响应: %d 字符, 速度: ~%.0f tokens/s",
                output_chars,
                output_tokens_est / elapsed if elapsed > 0 else 0,
            )
...
# 优化提示词 - 用于低分菜谱的完善
REFINE_SYSTEM_PROMPT = """你是一位资深中餐大厨和菜谱编辑专家。
你的任务是基于不完整的菜谱信息，生成一份专业、完整、可操作的菜谱。
...
- 补充原菜谱缺失的关键信息
- 保持菜品的正宗风味
- 标签必须有菜谱内容或公认菜品特征作为依据；不确定时少打，不要猜测
...
def normalize_recipe_tags(recipe: dict) -> dict:
    """Keep recipe tags unique and constrained to the Helios taxonomy."""
    tags = recipe.get("tags")
    if not isinstance(tags, dict):
        tags = {}

    normalized: dict[str, list[str]] = {}
    for tag_type, allowed in TAG_TAXONOMY.items():
        values = tags.get(tag_type, [])
        if not isinstance(values, list):
            values = []
        normalized[tag_type] = list(
            ...
            if result:
                ...
                if fetch_count % 20 == 0 or fetch_count == len(to_fetch):
                    logger.info(
                        ...
                        fetch_count,
                        len(to_fetch),
                        fetch_count / len(to_fetch) * 100,
                    )
            else:
                errors.append(futures[future].path)

    logger.info(
        "[FETCH] 完成! 成功: %d, 失败: %d, 跳过: %d", len(parsed), len(errors), skipped_count
    )
    if errors:
...
def process_node(state: DetectState) -> DetectState:
    """Evaluate recipes with the configured OpenRouter model."""
    import threading
    ...
    logger.info("=" * 50)
    process_model = get_model("process")
    logger.info("[PROCESS] 模型: %s", process_model)
    logger.info("[PROCESS] 待评估: %d 个菜谱", len(recipes))
    logger.info("[PROCESS] 并发数: %d", concurrency)

    llm = get_llm(process_model)
    ...
    def process_one(idx: int, recipe: dict) -> None:
        ...
        if cached_result and cached_result.get("processed_by") == process_model:
            with lock:
        ...
        try:
            messages = PROCESS_PROMPT.format_messages(
                ...
                tag_taxonomy=TAG_TAXONOMY_PROMPT,
            )
            response_content = invoke_llm(llm, messages, context=f"评估菜谱 {recipe_name}")
            ...
            processed_result = {
                **recipe,
                ...
                "refined": normalize_recipe_tags(result.get("recipe", {})),
                "processed_by": process_model,
            }
    ...
    logger.info(
        "[PROCESS] 高分 (≥8): %d 个 (%.1f%%)",
        high_score_count,
        high_score_count / len(recipes) * 100 if recipes else 0,
    )
    logger.info(
        "[PROCESS] 低分 (<8): %d 个 (%.1f%%)",
        low_score_count,
        low_score_count / len(recipes) * 100 if recipes else 0,
    )
...
def refine_node(state: DetectState) -> DetectState:
    ...
    def refine_one(idx: int) -> None:
        ...
        if cached_result and cached_result.get("refined_by") == refine_model:
            with lock:
                ...
                logger.info(
                    "[REFINE] [%d/%d] ↺ %s (从缓存恢复: %d 食材, %d 步骤)",
                    progress_count,
                    len(low_score_indices),
                    recipe_name,
                    ingredients_count,
                    steps_count,
                )
        ...
        try:
            ...
            response_content = invoke_llm(
                llm, messages, context=f"优化菜谱 {recipe_name} (原分数: {original_score})"
            )

            refined_recipe = normalize_recipe_tags(parse_llm_json(response_content))

            refined_result = {
                **recipe,
                "refined": refined_recipe,
                "refined_by": refine_model,
            }
    ...
    logger.info(
        "[REFINE] 成功率: %.1f%%",
        refined_count / len(low_score_indices) * 100 if low_score_indices else 0,
    )
...
def build_workflow() -> StateGraph:
    ...
    - fetch: 并发获取和解析菜谱
    - process: 使用 OpenRouter 配置的模型评估菜谱质量
    - refine: 使用 OpenRouter 配置的模型优化低分（<8分）菜谱
    - save: 保存结果到磁盘
    """
